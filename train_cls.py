...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Qwen3-Embedding-0.6B single-tower ESCI classifier (LoRA on q/v + classifier head).")
...
def main():
    ...
    print(f"Loading embedding model from: {args.base_model}")
    ...
    # ===== Apply LoRA on q_proj & v_proj =====
    print("Applying LoRA on q_proj & v_proj...")
    ...
    train_dataset = ESCIEmbClassifierDataset(
        df_train,
        tokenizer=tokenizer,
    ...
    print(f"Embedding classifier + tokenizer saved to: {output_dir}")
